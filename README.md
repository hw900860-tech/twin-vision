...
- **Store-and-forward resilience layer** — the airborne session keeps a **60 s ring buffer** of every frame it transmitted. The ground receiver is *ordered*: a frame that jumps ahead reveals a hole, the ground sends a `GAP_REQ` (0x05) upstream, and the airborne bursts the buffered window back down — so frames lost to a SATCOM drop or a radio outage are **recovered in strict sequence**, never duplicated, and the live display never freezes or jumps backward. Holes the ring can't cover (outage > 60 s) are counted `LOST`, not silently swallowed. The **relay records every binary frame** to `server/recordings/session-*.bin` (length-prefixed, direction-tagged) — the raw flight record the post-flight CSV debrief is generated from (`RELAY_RECORD=0` disables).
- **Test harness** — `scripts/fake_airborne.ts` (headless streaming peer), `scripts/e2e_datalink.ts` (rate/CRC/ACK smoke test) and `scripts/e2e_gap_recovery.ts` (suppresses a 15-frame window and asserts all 15 return in order, zero lost, recorder file grows).
...
4. Flip the sim's DATALINK modem to SATCOM → GCS latency jumps to ~250 ms with small loss. Switch to OUTAGE → NO LINK, frame age climbing. Wait a few seconds, flip back to LOS → the GCS **S&F REC** counter climbs as the airborne replays everything it buffered during the outage, then the stream resumes live. The sim's DATALINK panel shows the airborne **S&F BUFFER** depth and frames replayed.
